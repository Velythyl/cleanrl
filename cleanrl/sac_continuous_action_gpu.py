...
"""
    ...
    def make_random_action_sampler():
        low = torch.tensor(envs.single_action_space.low)
        high = torch.tensor(envs.single_action_space.high)

        def sampler():
            def do_one_env(i):
                return (low - high) * torch.rand(envs.single_action_space.shape[-1]) + high
            ret = torch.vmap(do_one_env, randomness="different")(torch.arange(envs.num_envs))
            return ret
        return sampler
